...
:choice-22
echo.
%<%:0e " Windows 8.1 Support "%>%
echo.
%<%:17 "Windows 8.1 installation media can be obtained from: "%>%
echo.
%<%:1f "Microsoft Volume Licensing Service Center (business customers): "%>%
%<%:1f "https://www.microsoft.com/licensing/servicecenter "%>%
echo.
%<%:1f "Microsoft Software Recovery site (with valid product key): "%>%
%<%:1f "https://www.microsoft.com/software-download/ "%>%
echo.
%<%:17 "Note: Windows 8.1 reached end of support on January 10, 2023 "%>%
echo.
pause
exit /b
...
:choice-21
echo.
%<%:0e " Windows 8 Support "%>%
echo.
%<%:17 "Windows 8 installation media can be obtained from: "%>%
echo.
%<%:1f "Microsoft Volume Licensing Service Center (business customers): "%>%
%<%:1f "https://www.microsoft.com/licensing/servicecenter "%>%
echo.
%<%:17 "Note: Windows 8 is no longer supported. Consider upgrading to Windows 10/11. "%>%
echo.
pause
exit /b
...
:choice-20
echo.
%<%:0e " Windows 7 Support "%>%
echo.
%<%:17 "Windows 7 installation media can be obtained from: "%>%
echo.
%<%:1f "Microsoft Volume Licensing Service Center (business customers): "%>%
%<%:1f "https://www.microsoft.com/licensing/servicecenter "%>%
echo.
%<%:1f "Microsoft Software Recovery site (with valid product key): "%>%
%<%:1f "https://www.microsoft.com/software-download/windows7 "%>%
echo.
%<%:1f "Alternative legitimate sources: "%>%
echo.
%<%:17 "Windows 7 SP1 Professional x64: "%>%
%<%:17 "https://archive.org/details/Win7_Pro_SP1_English_x64 "%>%
%<%:17 "Windows 7 SP1 Home Premium x64: "%>%
%<%:17 "https://archive.org/details/Windows7HomePremiumSP1x64 "%>%
echo.
%<%:17 "Note: Windows 7 reached end of support on January 14, 2020 "%>%
%<%:17 "Extended Security Updates ended January 10, 2023 "%>%
echo.
pause
exit /b
...
:choice-19
echo.
%<%:0e " Windows Vista Support "%>%
echo.
%<%:17 "Windows Vista reached end of life and is no longer supported. "%>%
%<%:17 "Installation media may be available from: "%>%
echo.
%<%:1f "Microsoft Volume Licensing Service Center (if you have access): "%>%
%<%:1f "https://www.microsoft.com/licensing/servicecenter "%>%
echo.
%<%:17 "Note: Windows Vista reached end of support on April 11, 2017 "%>%
echo.
pause
exit /b
...
:choice-18
echo.
%<%:0e " Windows XP Support "%>%
echo.
%<%:17 "Windows XP reached end of life and is no longer supported. "%>%
%<%:17 "Installation media is NOT officially available from Microsoft. "%>%
echo.
%<%:1f "For legacy system support, you may find archived media from: "%>%
%<%:17 "- Original retail/OEM installation discs "%>%
%<%:17 "- Volume Licensing Service Center (business customers) "%>%
%<%:17 "- Software preservation archives "%>%
echo.
%<%:17 "Note: Windows XP reached end of support on April 8, 2014 "%>%
%<%:1f "Use only for legacy system support or virtual machines "%>%
echo.
pause
exit /b
...
if defined XML echo;%XML% & call :CHECK_URL "%XML%" && call :DOWNLOAD "%XML%" products%VID%.xml
if defined CAB echo;%CAB% & call :CHECK_URL "%CAB%" && call :DOWNLOAD "%CAB%" products%VID%.cab
...
function DOWNLOAD ($u, $f, $p = (get-location).Path) {
  ...
  if (([IO.FileInfo]$file).Exists) {return}; write-host -fore Red " $f download failed "
  write-host -fore Yellow " This may indicate: "
  write-host -fore Yellow " - Network connectivity issues "
  write-host -fore Yellow " - Microsoft has moved or removed the file "
  write-host -fore Yellow " - Server maintenance in progress "
  write-host -fore Yellow " Please check your internet connection and try again later "
  write-host ""
  pause
} #:DOWNLOAD:# try download url via bits, net, and http/https - enhanced error handling

:CHECK_URL
set ^ #=;$u='%~1'; CHECK_URL $u
set ^ #=& set "0=%~f0"& set 1=;CHECK_URL %*& powershell -nop -c "%#%"& exit /b %errorlevel%
function CHECK_URL ($u) {
  write-host -fore Gray "Checking URL availability: $u"
  try {
    ...
    $response.Close()
    write-host -fore Green "URL is accessible"
    return 0
  } catch {
    write-host -fore Red "URL check failed: $($_.Exception.Message)"
    write-host -fore Yellow "This Windows version may no longer be available from Microsoft"
    write-host -fore Yellow "Please check for alternative download sources"
    return 1
  }
